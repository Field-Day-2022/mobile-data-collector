...
const toeClipCodeReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case ACTIONS.UPDATE_NEW_TOE_CLIP_CODE:
            ...
            for (
                let i = newDateState[action.payload.site][action.payload.species].length - 1;
                i >= 0;
                i--
            ) {
                if (
                    action.payload.toeClipCode ===
                    newDateState[action.payload.site][action.payload.species][i].toeClipCode
                ) {
                    newDateState[action.payload.site][action.payload.species][i].entry_id =
                        entry_id;
                    break;
                }
            }
