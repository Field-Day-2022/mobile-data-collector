...
const NewDataEntry = (props) => {
    ...
    const getCollectedSpeciesCount = (animalName) => {
        ...
        if (animalName === CONSTANTS.ARTHROPOD) {
            ...
            arthropodArray.forEach((elem) => {
                arthroCount = elem.reduce((acc, curr) => Number(acc) + Number(curr), arthroCount);
            });
            return arthroCount;
        } else {
    ...
    useEffect(() => {
        ...
        const clearedObject = currentSessionDataObjKeyArray.reduce((acc, curr) => {
            return {
                ...acc,
                [curr]: '',
            };
    ...
    return (
        <div className="home-page-backing">
            <div className="header">
                <h1>
                    <span className="spacer">{currentSite} New Data Entry</span>
                </h1>
            </div>
                            ...
                            <Form noValidate validated={validated} onSubmit={handleSubmit}>
                                {fieldHasError('animal') && (
                                    <p className="error-class">
                                        <span className="error-text">{errors.animal}</span>
                                    </p>
                                )}
