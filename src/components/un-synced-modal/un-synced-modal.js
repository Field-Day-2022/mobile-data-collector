...
export default (props) => {
    ...
    const generateTableHeaders = (json) => {
        if (json) {
            ...
            return tableHeaders.map((key, index) => (
                <th key={`${key}-${index}`}>{_.startCase(_.toLower(key))}</th>
            ));
        }
    };
    ...
    return (
        <Modal show={props.show} onHide={props.onHide}>
            ...
            <Modal.Body id="unsynced-history-modal">
                <Container>
                    <Table>
                        <thead>
                            <tr>{generateTableHeaders(entry_json)}</tr>
                        </thead>
                        <tbody>
                            <tr>{generateTableData(entry_json)}</tr>
                        </tbody>
                    </Table>
                </Container>
            </Modal.Body>
