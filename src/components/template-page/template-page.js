...
const TemplatePage = (props) => {
    ...
    const reliesOnContainerArr = fields.filter((field) =>
        field.hasOwnProperty(CONSTANTS.RELIES_ON)
    );
    ...
    const validate = () => {
        ...
        for (const prop in reduxErrorState) {
            if (reduxErrorState.hasOwnProperty(prop)) {
                ...
                if (prop === 'Regen Tail?') {
                    if (
                        parseFloat(currentSession['OTL(mm)']) >
                        parseFloat(currentSession['VTL(mm)'])
                    ) {
        ...
        return (
            Object.values(errState).filter(
                (val) => _.toLower(String(val)) === _.toLower(CONSTANTS.REQUIRED)
            ).length <= 0
        );
    };
    ...
    const onHandleSubmit = (event) => {
        ...
        if (isFormValid) {
            if (onVerifyPage) {
                ...
                const getProject_Id = _.find(
                    all_projects,
                    (project) => project.project_name === currentLocation
                );
    ...
    const buildDynamicForm = (fields) => {
        return <FormFactory fields={fields} />;
    };
                ...
                <div>
                    <div className="header">
                        <h1>
                            <span className="spacer">
                                {form_name_n === 'Session'
                                    ? `${currentLocation} New Data`
                                    : `${currentSite} New ${form_name_n} Data`}
                            </span>
                        </h1>
                    </div>
                    <div className="center-column">
                        <Container className="amphibian-form">
                            <Row>
                                <Col>{buildDynamicForm(fields)}</Col>
                            </Row>
