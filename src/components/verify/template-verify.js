    ...
    const fieldJSX = () => {
        ...
        if (props.formName === CONSTANTS.ARTHROPOD) {
            arr = Object.keys(currentFormData)
                ...
                .filter(
                    (keyObj) =>
                        keyObj.key === CONSTANTS.FENCE_TRAP ||
        ...
        return arr.map((el) => {
            return (
                <div className="verify-section" key={el.key}>
                    <p className="verify-key">{el.key}</p>
    ...
    return (
        <div className="home-page-backing">
            <div className="header">
                <h1>
                    <span className="spacer">Verify</span>
                </h1>
            </div>
            <div className="center-column">
                <Container id={`${props.formName}`}>
                    <Row>
                        <Col>{fieldJSX().map((field) => field)}</Col>
                    </Row>
                </Container>
