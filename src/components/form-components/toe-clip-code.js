...
import { ADD_USED_CODE, REMOVE_USED_CODE } from '../../redux/actions/toe-clip-code-actions';
...
const ToeClipCodeWrapper = ({ field }) => {
    ...
    const recaptureLookUp = async () => {
        if (
            !(
                redux_field[CONSTANTS.SPECIES_CODE] &&
                redux_field[CONSTANTS.TOE_CLIP_CODE] &&
                currentSite
            )
        ) {
            return;
        }
        let captureHistoryEntryID = currentToeClipCodes[redux_field[CONSTANTS.SPECIES_CODE]].filter(
            (o) => o.toeClipCode === redux_field[CONSTANTS.TOE_CLIP_CODE]
        );
        ...
        for (const entry_id of captureHistoryIDs) {
            ...
            listDataWithoutDate.Date = new Date(sessionInfo.date_created * 1000).toLocaleDateString(
                'en-US'
            );
    ...
    const codeNotUsed = (code) => {
        ...
        if (currentToeClipCodes[redux_field[CONSTANTS.SPECIES_CODE]]) {
            currentToeClipCodes[redux_field[CONSTANTS.SPECIES_CODE]].forEach(
                (toeClipCodeObject) => {
    ...
    const generateToeClipCode = async () => {
        ...
        await getAnswerSetByName('toe clip codes').then((value) => {
            for (let currentCode of value.answers) {
                ...
                if (
                    !toeClipCodesStrings.includes(toeClipCode) &&
                ...
                <div
                    className={
                        redux_field[CONSTANTS.RECAPTURE]
                            ? 'toe-clip-question-column'
                            : 'toe-clip-question-row'
                    }
                >
