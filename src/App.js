...
function App() {
    ...
    useEffect(() => {
        onSnapshot(doc(db, 'Metadata', 'LizardData'), (snapshot) => {
            console.log(`Local last edit time: ${new Date(lastEditTime).toLocaleString()}, Server last edit time: ${new Date(snapshot.data().lastEditTime).toLocaleString()}`)
            if (snapshot.data().lastEditTime !== lastEditTime) {
                console.log(
                    `fetching new/modified lizard data from ${new Date(lastEditTime).toDateString()} to ${new Date(snapshot.data().lastEditTime).toDateString()}`
                );
        ...
        onSnapshot(collection(db, 'AnswerSet'), (snapshot) => {
            console.log(`Answer set loaded from ${snapshot.metadata.fromCache ? 'cache' : 'server'}`);
    ...
    return (
        <motion.div className="font-openSans  overflow-hidden  absolute  flex  flex-col  items-center  text-center  justify-start  inset-0  bg-white">
            <AnimatePresence mode="wait">
                {answerSetLoading ? <LoadingScreen /> : <AppWrapper>
